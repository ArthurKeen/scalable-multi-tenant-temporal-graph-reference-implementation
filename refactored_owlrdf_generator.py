"""
Refactored W3C OWL Multi-Tenant Generator

Clean, modular implementation using centralized configuration management,
eliminated hard-wiring, and improved code quality.
"""
...
class RefactoredTenantDataGenerator:
    """Refactored W3C OWL compliant tenant data generator with centralized configuration."""
    ...
    def generate_device_proxies(self) -> Tuple[List[Dict], List[Dict]]:
        ...
        self.logger.info(f"Generated {len(device_ins)} DeviceIn and {len(device_outs)} DeviceOut entities")
        return device_ins, device_outs
    
    def generate_device_configurations(self, device_ins: List[Dict]) -> Tuple[List[Dict], List[Dict]]:
        """Generate W3C OWL compliant device configurations with temporal versioning."""
        ...
        for i, device_in in enumerate(device_ins):
            device_type_str = device_in["deviceType"]
    ...
    def _create_version_edges(self, proxy_key: str, device_key: str, 
        ...
        version_in = DocumentEnhancer.create_edge_document(
            ...
            from_collection=self.app_config.get_collection_name("device_ins"),
            from_key=proxy_key,
            to_collection=self.app_config.get_collection_name("devices"),
            to_key=device_key,
            from_type="DeviceIn",
            to_type="Device",
        ...
        version_out = DocumentEnhancer.create_edge_document(
            ...
            from_collection=self.app_config.get_collection_name("devices"),
            from_key=device_key,
            to_collection=self.app_config.get_collection_name("device_outs"),
            to_key=proxy_key,
            from_type="Device",
            to_type="DeviceOut",
            tenant_config=self.tenant_config,
        ...
        while len(connections) < self.tenant_config.num_connections and attempts < self.limits.MAX_GENERATION_RETRIES:
            ...
            if from_device["_key"] != to_device["_key"]:  # Prevent self loops
                ...
                connection = DocumentEnhancer.create_edge_document(
                    key=connection_key,
                    from_collection=self.app_config.get_collection_name("device_outs"),
                    from_key=from_device["_key"],
                    to_collection=self.app_config.get_collection_name("device_ins"),
    ...
    def generate_has_location_edges(self, device_outs: List[Dict], locations: List[Dict]) -> List[Dict[str, Any]]:
        ...
        for device in device_outs:
            ...
            has_location = DocumentEnhancer.create_edge_document(
                ...
                to_collection=self.app_config.get_collection_name("locations"),
        ...
        while len(has_software) < self.tenant_config.num_has_software and attempts < self.limits.MAX_GENERATION_RETRIES:
            ...
            if device["deviceType"] != "router":
                ...
                has_software_edge = DocumentEnhancer.create_edge_document(
                    ...
                    from_collection=self.app_config.get_collection_name("device_outs"),
                    from_key=device["_key"],
                    to_collection=self.app_config.get_collection_name("software"),
    ...
    def generate_all_data(self) -> Dict[str, Any]:
        ...
        print(f"✅ Generated {total_documents} W3C OWL compliant documents")
...
def generate_refactored_multi_tenant_demo(environment: str = "production"):
    """Generate refactored W3C OWL compliant multi-tenant demo."""
    
    print("🔧 Refactored W3C OWL Multi-Tenant Generation")
    print("=" * 60)
    ...
    for tenant_config in tenant_configs:
        generator = RefactoredTenantDataGenerator(tenant_config, environment)
    ...
    registry_path = app_config.paths.data_directory / "tenant_registry_refactored.json"
    with open(registry_path, "w") as f:
        json.dump(tenant_registry, f, indent=2)
    
    print(f"\n🎉 Refactored W3C OWL generation completed!")
    ...
    print(f"🔧 Architecture: Centralized configuration, eliminated hard-wiring")
    
    return results


if __name__ == "__main__":
    results = generate_refactored_multi_tenant_demo()
    print(f"\n✅ Ready for refactored W3C OWL compliant deployment!")
